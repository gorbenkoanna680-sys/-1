...
if attack_type != st.session_state.last_attack:
    if current_log_event:
        ...
        st.session_state.logs.append("Атаку скасовано. Мережа повернулася до нормального стану.")
...
col_m1.metric("📦 Маршрут (Дейкстра)", " -> ".join(path_nodes) if has_path else "ШЛЯХ ВІДСУТНІЙ")
col_m2.metric("⏱ Поточна затримка (Latency)", f"{path_latency} ms" if has_path else "∞ (Недоступно)")
col_m3.metric("🚨 Компонент зв'язності", f"{components} (Розпад!)" if components > 1 else f"{components}")
col_m4.metric("📊 Щільність графа", f"{density:.2f}")

st.markdown("---")

# ХАРАКТЕРИСТИКИ ГРАФА (Вимога пункту 2 зауважень)
st.subheader("📊 Основні аналітичні характеристики графа мережі")
col_char1, col_char2, col_char3 = st.columns(3)
with col_char1:
    st.write(f"**Кількість активних вузлів (вершин):** {num_nodes}")
    st.write(f"**Кількість активних ліній (ребер):** {num_edges}")
with col_char2:
